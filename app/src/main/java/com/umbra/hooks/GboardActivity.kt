...
class GboardActivity : androidx.appcompat.app.AppCompatActivity() { // Changed to AppCompatActivity for lifecycleScope
    ...
    private fun saveSettings() {
        ...
        if (editor.commit()) {
            fixPermissions()
            killGboard() // Attempt to force stop
        } else {
            Toast.makeText(this, "Failed to save settings", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun runClipboardTest(count: Int) {
        ...
        // Use Coroutine to copy items with a small delay
        lifecycleScope.launch {
            for (i in 1..count) {
                val text = "Test Item #$i - (${System.currentTimeMillis()})"
                val clip = ClipData.newPlainText("UmbraTest", text)
                clipboard.setPrimaryClip(clip)
                delay(100) // 100ms delay to ensure Gboard registers the copy
            }
            Toast.makeText(this@GboardActivity, "Done! Check Gboard Clipboard.", Toast.LENGTH_LONG).show()
        }
    }
    ...
    private fun killGboard() {
        ...
        try {
            ...
            // Root failed or not available
        }

        // Method 2: Fallback to App Info Page
        Toast.makeText(this, "Root unavailable. Please Force Stop manually.", Toast.LENGTH_LONG).show()
        try {
            val intent = Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS).apply {
                data = Uri.fromParts("package", gboardPackage, null)
